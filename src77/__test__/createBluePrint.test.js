...
it("render create Blueprint modal", async () => {
  // notice the async
  await act(async () => {
    ...
    expect(getByTestId("create-blueprint-trigger")).toBeInTheDocument();
  });

  // this section will run after the effects within TheComponent were triggered
});

test("should render header on create blueprint", async () => {
  ...
  await act(async () => {
    ...
    await userEvent.click(getByTestId("create-blueprint-trigger"));
    // const response = getByTestId("header");
    expect(getByTestId("header")).toBeInTheDocument();
  });
});

test("should rendr create blueprint form", async () => {
  // window.ResizeObserver = ResizeObserver;
  await act(async () => {
    // this is kind of ugly, but it works.
    const setShowBluePrintNotification = jest.fn();
...
test("should rendr create blueprint form inputes", async () => {
  // window.ResizeObserver = ResizeObserver;
  await act(async () => {
    // this is kind of ugly, but it works.
    const setShowBluePrintNotification = jest.fn();
